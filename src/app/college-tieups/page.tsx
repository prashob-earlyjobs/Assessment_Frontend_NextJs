        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {colleges.map((college) => (
            <div key={college.id} className="college-card bg-gray-100 p-4 rounded-lg shadow-md text-center hover:scale-105 transition-transform duration-300 border-t-3 border-orange-500 hover:border-blue-800">
              {college.logo_url ? (
                <img src={college.logo_url} alt={`${college.name} logo`} className="mx-auto h-18 mb-4 bg-white" />
              ) : (
              ...
              <button className="mt-4 px-4 py-2 bg-yellow-500 text-white rounded-full hover:bg-yellow-600">Learn More</button>
            </div>
          ))}
        </div>
      </div>
